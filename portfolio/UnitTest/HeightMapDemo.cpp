...
void HeightMapDemo::Update()
{
	...
	//{
	//	function<void(wstring)> f = bind(&HeightMapDemo::LoadTexture, this, placeholders::_1); //placeholders:인자 갯수
...
//void HeightMapDemo::LoadTexture(wstring file)
//{
...
void HeightMapDemo::CreateVertex()
{
	////      1     3       
	////      
	////      0     2
...
void HeightMapDemo::CreateVertexEnemy(ID3D11Buffer** vertexBuffer)
{
	////Local
...
bool HeightMapDemo::CheckRectCollision(Matrix player, Matrix enemy)
{
	return IsIntersect(player._41 - 0.5f, player._42 - 0.5f, 0.5f, 0.5f, enemy._41 - 0.5f, enemy._42-0.5f, 0.5f, 0.5f);
	return false;
}
bool HeightMapDemo::IsIntersect(int x1, int y1, int w1, int h1, int x2, int y2, int w2, int h2)
{ 
