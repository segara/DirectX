...
{
	Matrix Scale, Rotation, Transform;


	
		if (Keyboard::Get()->Press(VK_RIGHT)) {
			z_rotation += 20.0f * Time::Delta();
		}
	...
	D3DXMatrixRotationZ(&Rotation,z_rotation * (Math::PI / 180.0f));
...
void RotationDemo::CreateVertex()
{
	//      14     5      
	//      
	//      0     23
...
void RotationDemo::CreateVertexEnemy(ID3D11Buffer** vertexBuffer)
{
	//Local
...
bool RotationDemo::CheckRectCollision(Matrix player, Matrix enemy)
{
	return IsIntersect(player._41 - 0.5f, player._42 - 0.5f, 0.5f, 0.5f, enemy._41 - 0.5f, enemy._42-0.5f, 0.5f, 0.5f);
	return false;
}
bool RotationDemo::IsIntersect(int x1, int y1, int w1, int h1, int x2, int y2, int w2, int h2)
{ 
