...
void ProjectorDemo::Update()
{
	...
	static float alpha = 0.52f;
	ImGui::SliderFloat("RefractionAlpha", &alpha, 0, 1);
	shader->AsScalar("RefractionAlpha")->SetFloat(alpha);

	static float cubemap_amount = 0.52f;
	ImGui::SliderFloat("CubeMapAmount", &cubemap_amount, 0, 1);
	shader->AsScalar("CubeMapAmount")->SetFloat(cubemap_amount);

	static float cubemap_bias = 0.52f;
	ImGui::SliderFloat("CubeMapBias", &cubemap_bias, 0, 1);
	shader->AsScalar("CubeMapBias")->SetFloat(cubemap_bias);

	static float cubemap_scale = 0.52f;
	ImGui::SliderFloat("CubeMapScale", &cubemap_scale, 0, 1);
	shader->AsScalar("CubeMapScale")->SetFloat(cubemap_scale);
	////SpotLight
	//{
