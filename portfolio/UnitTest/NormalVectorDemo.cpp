...
void NormalVectorDemo::Update()
{
	...
	//{
	//	function<void(wstring)> f = bind(&NormalVectorDemo::LoadTexture, this, placeholders::_1); //placeholders:인자 갯수
...
//void NormalVectorDemo::LoadTexture(wstring file)
//{
...
void NormalVectorDemo::CreateVertex()
{
	////      1     3       
	////      
	////      0     2
...
void NormalVectorDemo::CreateVertexEnemy(ID3D11Buffer** vertexBuffer)
{
	////Local
...
bool NormalVectorDemo::CheckRectCollision(Matrix player, Matrix enemy)
{
	return IsIntersect(player._41 - 0.5f, player._42 - 0.5f, 0.5f, 0.5f, enemy._41 - 0.5f, enemy._42-0.5f, 0.5f, 0.5f);
	return false;
}
bool NormalVectorDemo::IsIntersect(int x1, int y1, int w1, int h1, int x2, int y2, int w2, int h2)
{ 
