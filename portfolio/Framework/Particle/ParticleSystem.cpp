 ...
 * .MapVertices
 * 
 * leadCount를 gpuCount가 따라가며 두개의 차이만큼 Map을 실행(gpu memory로 복사)
...
void ParticleSystem::MapVertices()
{
	...
	if (gpuCount < leadCount)
	{
		...
		{
			UINT start = gpuCount * 4; //정점이 4개이므로
			...
			//vertices는 파티클데이터 배열이므로 배열시작지점에서 start 인덱스를 더함
			memcpy(p, vertices + start, size);
		}
		D3D::GetDC()->Unmap(vertexBuffer->Buffer(),0);
	}
	else //원형큐를 한번 돌아 0을 지난 상황
	{
		D3D::GetDC()->Map(vertexBuffer->Buffer(), 0, D3D11_MAP_WRITE_NO_OVERWRITE, 0, &subResource);
		{
			UINT start = gpuCount * 4; //정점이 4개이므로
...
void ParticleSystem::SetTexture(wstring texture){
	SafeDelete(map);
	map = new Texture(texture);
}
