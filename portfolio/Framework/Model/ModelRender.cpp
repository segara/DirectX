...
void ModelRender::CreateTexture()
{
	...
	for (UINT i = 0; i < MAX_MODEL_INSTANCE; i++)
	{
		for (UINT b = 0; b < model->BoneCount(); b++)
		{
			...
			bones[b] = parent;//현재 bone을 저장하거나 (부모가 없을시)부모를 저장해둠, bones[parentIndex]을 위해
			//자신과 부모의 matrix를 곱하여 최종 transform
...
void ModelRender::UpdateTransform(UINT instanceId, UINT boneIndex, Transform& transform)
{
	...
	//todo: tempBoneIndex 말고 child index 써보기
	for (ModelBone* child : bone->Childs())
	{
		...
		tempBoneIndex++; //
		printf("%d",tempBoneIndex);
		printf("%d", child->Index());
		Matrix temp = boneTransforms[instanceId][child->Index()] * invParent; //자기자신 transform
		boneTransforms[instanceId][child->Index()] = temp * destMatrix * parent;
	}
