...
struct TweenFrame
{
    float TakeTime; //1이 완료
    float TweenTime; //경과시간 저장용
    float RunningTime; //tween이 진행되는 시간
    float Padding;
...
void SetAnimationWorld(inout matrix world, VertexModel input)
{
    ...
    for (int i = 0; i < 4;i++)
    {
        //sample은 확대 축소가 일어나므로 데이터에 변형이 일어나서 사용하지 않고 load 사용
        ...
        transform += mul(weights[i], currAnim); //현재 계산된 본의 움직임과 영향정도(weight)를 곱해준후 더함, 따라서 각 본의 움직임에 영향을 받은 해당 정점이 움직임이 계산됨
    }
